...
const vec3[] grad = vec3[](vec3(1, 1, 0), vec3(-1, 1, 0), vec3(1, -1, 0), vec3(-1, -1, 0), vec3(1, 0, 1), vec3(-1, 0, 1), vec3(1, 0, -1), vec3(-1, 0, -1), vec3(0, 1, 1), vec3(0, -1, 1), vec3(0, 1, -1), vec3(0, -1, -1));
...
bool random_bool(ivec2 p) {
    ...
    return simplex3d(vec3(vec2(p) * .1, time * 0.3)) > 0.;
}
