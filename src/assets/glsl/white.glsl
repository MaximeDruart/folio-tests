...
void main() {
    ...
    float incTime = floor(time * 16.) / 16.;
    incTime = incTime * 2.;

    // vec3 color = vec3(circle(fpos, 0.5));
    vec3 color = vec3(1.);

    float noise = 1. - snoise(vec3(ipos, incTime));
    noise = step(0.07, noise);

    float noise2 = random(vec2(sin(time * 100.) * 0.01));
    noise2 = step(0.001, noise2);

    color = vec3(noise) * vec3(noise2);
